...
class Home extends Component {
  ...
  onClickArtCategory = (event) => {
    ...
    let updatedCategories = [...this.state.selectedCategories];
    if (isChecked) {
  ...
  onSelectDateRange = (range) => {
    ...
    if (this.state.dateRangeValue.end !== null) {
      ...
      this.state.filteredArtworks.find((artwork) => {
        if (
          artwork.created_date >= convertDateToTimestamp(range.from) &&
          artwork.created_date <= convertDateToTimestamp(range.to)
        ) {
          artworkSearchResult.push(artwork);
        }
      });
    ...
    return (
      <Layout color="White">
        <div className={classes.Home}>
          <div className={classes.SearchDiv}>
            <div className={classes.FilterDiv}>
              <div className={classes.SearchBar}>
                <Search
                  keypressed={this.onSearchHandler}
              ...
              <div className={classes.Filters}>
                <DropDownButton
                  label="Art Categry"
                  isActive={this.state.selectedCategories.length > 0}
                >
                  <CheckList
                    list={this.props.artCategories}
                    clicked={this.onClickArtCategory}
                  />
                </DropDownButton>
                <DropDownButton
                  label={
                    this.state.dateRangeValue.end
                      ? convertToISO(this.state.dateRangeValue.start) +
                        " to " +
                        convertToISO(this.state.dateRangeValue.end)
                      : "Date"
                  }
                  isActive={this.state.dateRangeValue.end !== null}
                >
                ...
                <DropDownButton
                  label={
                    this.state.selectedPrice === 0
                      ? "Price"
                      : "$1-" + this.state.selectedPrice
                  }
                  isActive={this.state.selectedPrice !== 0}
                >
                  <div className={classes.PriceRangeSlider}>
                    <RangeSlider
                      selectRange={this.onSelectPriceRange}
                      max={this.state.priceRangeValue.max}
                      min={this.state.priceRangeValue.min}
                      default={this.state.priceRangeValue.max}
                    />
                  </div>
                </DropDownButton>
                <DropDownButton
                  label={this.state.sortType}
                  isActive={this.state.sortType !== "Sort By"}
                >
          ...
          <section className={classes.Artworks}>{arts}</section>
          <div className={classes.ArtworksPagination}>
            {artSource.length > 0 ? (
              <Pagination
                data={this.props.arts}
                parentCallback={this.callBackFuntion}
              />
            ) : null}
          </div>
        </div>
